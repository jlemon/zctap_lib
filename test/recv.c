...
{
	struct iovec *iov[BATCH_SIZE], *miov;
	...
	for (i = 0; i < count; i++) {
		...
		miov = (struct iovec *)&meta->data[0];
		for (f = 0; f < meta->iov_count; f++) {
			pkt_dump(&miov[f]);
			rx_bytes += miov[f].iov_len;
			zctap_recycle_buffer(ifq, miov[f].iov_base);
		}
...
test_recv(const char *hostname, short port)
{
	...
	sz = opt.nentries * 256;
	pktbuf = zctap_alloc_memory(sz, MEMTYPE_HOST);
	CHECK(pktbuf);
	CHK_ERR(zctap_add_meta(skq, fd, pktbuf, sz, opt.nentries, 256));
